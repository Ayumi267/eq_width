...
# about not irradiate
Google_Drive_datadir = "/Users/kobayashiaiketsu/Library/CloudStorage/GoogleDrive-a.kobayashi267@gmail.com/.shortcut-targets-by-id/1ZkGf0KEeIpXHgpjgZ6RIBuECFYX3dERR/study/data/experiment/spectra/kobayashi/NaCl/"
DATE = "2024-09-13"
FILE_ID_IRRADIATED = "9"
DATA_FILE_PATH_IRRADIATED = f"{Google_Drive_datadir}/{DATE}/{DATE}_{FILE_ID_IRRADIATED}.txt"
print(DATA_FILE_PATH_IRRADIATED)
FILE_ID_BEFORE_IRRADIATED = "3"
DATA_FILE_PATH_BEFORE_IRRADIATED = f"{Google_Drive_datadir}/{DATE}/{DATE}_{FILE_ID_BEFORE_IRRADIATED}.txt"
print(DATA_FILE_PATH_BEFORE_IRRADIATED)
outfile_fig = Google_Drive_datadir + DATE + \
    "/plot_eqwidth/" + 'eqwidth_' + \
    str(FILE_ID_IRRADIATED) + 'divide' + \
    str(FILE_ID_BEFORE_IRRADIATED) + '.png'
IRRADIATION_TIME = "e-_222min"
...
wavelengths_irradiated, intensities_irradiated = read_data(
    DATA_FILE_PATH_IRRADIATED)
wavelengths_before_irradiated, intensities_before_irradiated = read_data(
    DATA_FILE_PATH_BEFORE_IRRADIATED)

# Normalize intensities
norm_intensities_irradiated = normalize_data(
    intensities_irradiated, target_index)
norm_intensities_before_irradiated = normalize_data(
    intensities_before_irradiated, target_index)


wavelengths = wavelengths_irradiated
intensities = norm_intensities_irradiated/norm_intensities_before_irradiated


# プロットするwavelengthの範囲を設定--カットする位置で適宜調整
...
def gaussian_fitting(wavelengths_cut, peak_positions_list, peak_list, opt_dimension, intensities_divided_by_fitted_values):
   ...
   # ガウシアンフィッティング時に初期値として自動補正後のピーク位置を使用
    params_gauss, cov_gauss = curve_fit(
        Gaussian_list, wavelengths_cut, intensities_divided_by_fitted_values[opt_dimension - 2], gaussian_params, maxfev=1000000)
...
peak_positions_list = [460, 590, 720]
# peak_positions_list = [460, 590, 720, 830]  # ピーク位置を適宜追加
peak_positions_list = [460.757, 717.768]  # ピーク位置を適宜追加
# peak_positions_list = [600.694]
# peak_positions_list = [600, 720, 830]
peak_list = [0., 0., 0., 0.]
...
t2 = str(wavelength_range[0])+'-' + str(wavelength_range[1]) + ' nm and upto '+str(
    opt_dimension)+'deg fit(' + str(x1_min)+'-'+str(x1_max)+' and '+str(x2_min)+'-'+str(x2_max) + ' nm)'
...
for i in range(num_func):
    ...
    # # ガウシアン関数のピーク位置を中心として、幅の2倍を塗る範囲を指定
    # mask = (wavelengths_cut >= ctr - 2 *
    #         wid) & (wavelengths_cut <= ctr + 2 * wid)
    # y_list[mask] = amp * np.exp(-((wavelengths_cut[mask] - ctr) / wid) ** 2)
    # 面積に基づいて薄く色付け
